...
// THEME: Gerenciamento de tema (light / dark) com persistência e preferências do sistema
...
function applyTheme(theme) {
  if (theme === "dark") {
    ...
    // Atualiza meta theme-color (melhora experiência em mobile)
...
function initTheme() {
  ...
  const theme = saved || detectSystemPref();
  applyTheme(theme);

  const toggle = document.getElementById("theme-toggle");
  if (toggle) {
    toggle.addEventListener("click", () => {
      const current = localStorage.getItem(THEME_KEY) || detectSystemPref();
  ...
  // Escuta mudanças no sistema e aplica somente se usuário não tiver preferência salva
...
function keepScreenOn() {
  ...
  navigator.wakeLock
    ...
    .catch((error) => {
      console.warn('Não foi possível ativar o Wake Lock:', error);
    });
}
...
// Inicia a aplicação quando o DOM estiver disponível
